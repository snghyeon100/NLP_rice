"""End-to-end grad_diff unlearning runner (no KL term).

Run from the repo root with:
    python unlearning_methods/unlearn_grad_diff/train.py
"""
...
from unlearning_methods.unlearn_grad_diff.dataloader import GradDiffDataset, grad_diff_collator
from unlearning_methods.unlearn_grad_diff.loss import compute_grad_diff_loss
...
class GradDiffTrainer(Trainer):
    def __init__(self, *args, eval_cfg=None, tokenizer=None, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        loss, outputs = compute_grad_diff_loss(model, inputs)
...
def main(cfg):
    ...
    # grad_diff는 oracle model 불필요 — GPU 1개만 사용
